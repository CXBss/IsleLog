...
import 'article_conflict_editor_page.dart';

/// 文章冲突概览页
///
/// 展示本地与远端在标题和正文上的差异，供用户了解冲突详情后进入编辑页处理。
///
/// [article]：本地 ArticleEntry（title/content 为本地编辑后版本，originalTitle/originalContent 为编辑前快照）
/// [remoteTitle]、[remoteContent]：从服务端拉取的最新值
/// [onResolved]：冲突处理完成后的回调（传入最终 title, content）
class ArticleConflictOverviewPage extends StatelessWidget {
  ...
  const ArticleConflictOverviewPage({
    super.key,
                    ...
                    child: ElevatedButton.icon(
                      ...
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => ArticleConflictEditorPage(
                              article: article,
                              remoteTitle: remoteTitle,
                              remoteContent: remoteContent,
                              onResolved: (resolvedTitle, resolvedContent) async {
                                await onResolved(resolvedTitle, resolvedContent);
  ...
  bool get _hasTitleDiff {
    final base = article.originalTitle ?? '';
  ...
  Widget _buildFieldDiff(BuildContext context,
      {required String base, required String local, required String remote}) {
