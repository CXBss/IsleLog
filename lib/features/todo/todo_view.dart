                  ...
                  onPressed: () async {
                    ...
                    if (mounted) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(count > 0 ? '扫描完成，更新了 $count 条' : '扫描完成，状态均正确'),
...
class _TodoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < lines.length; i++) {
      final line = lines[i];
      if (line.contains('- [ ]')) {
        todoItems.add(_TodoItem(lineIndex: i, text: line.replaceFirst(RegExp(r'^.*- \[ \]\s*'), ''), done: false));
      } else if (RegExp(r'- \[[xX]\]').hasMatch(line)) {
        todoItems.add(_TodoItem(lineIndex: i, text: line.replaceFirst(RegExp(r'^.*- \[[xX]\]\s*'), ''), done: true));
      }
    }
...
class _TodoItem {
  ...
  final String text;
  final bool done;
  const _TodoItem({required this.lineIndex, required this.text, required this.done});
}
